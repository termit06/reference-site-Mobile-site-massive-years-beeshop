...
const Task = () => {
    const searchArchive = (from: '2025-02-01', to: '2025-02-28', data: any) => {
        let result = [];
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < data[i].versions.length; j++) {
                let startVersionDate = new Date(data[i].versions[j].from).getTime();
                let endVersionDate;

                if (j + 1 === data[i].versions.length) {
                    break;
                } else {
                    endVersionDate = new Date(data[i].versions[j + 1].from).getTime();
                }

                if (startDate <= startVersionDate && endDate <= endVersionDate) {
                    result.push(data[i].versions[j].text);
                }
            }
        }
    ...
    searchArchive('2025-02-01', '2025-02-28', arrayData);
    return(<Box>
        <Typography>Начинающий архивариус
